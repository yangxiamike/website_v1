...
import { MobileMarquee, MobileRail, PageHero, SectionHeading } from '../components/common';
    ...
    <div className="pt-[80px]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <MobileRail>
            {cs.challenges.map((ch, i) => (
              <div key={`${ch.title}-mobile`} className="w-[80vw] min-w-[276px] max-w-[320px] border border-gray-200 bg-white p-5">
                <div className="mb-3 flex items-center gap-3">
                  <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center bg-brand-red text-xs font-bold text-white">
                    {i + 1}
                  </span>
                  <h3 className="text-[15px] font-semibold text-text-primary">{ch.title}</h3>
                </div>
                <p className="text-sm leading-relaxed text-text-secondary">{ch.desc}</p>
              </div>
            ))}
          </MobileRail>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <MobileRail>
            {cs.productsUsed.map((pu) => (
              <div key={`${pu.id}-mobile`} className="w-[74vw] min-w-[248px] max-w-[288px] border border-gray-200 bg-white p-5 transition-colors hover:border-brand-red/30">
                <div className="mb-4 flex h-32 items-center justify-center bg-gray-50 p-4">
                  <img src={pu.image} alt={pu.name} className="max-h-full max-w-full object-contain" />
                </div>
                <h4 className="text-[15px] font-semibold text-text-primary">{pu.name}</h4>
                <p className="mt-1 text-xs text-text-muted">{pu.series}</p>
      ...
      <section className="bg-white py-10 lg:py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <MobileMarquee
            items={cs.results}
            getKey={(result) => result.label}
            renderItem={(r) => (
              <div className="w-[78vw] min-w-[268px] max-w-[320px] border border-gray-200 bg-white p-5">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <MobileRail className="lg:hidden">
            {cs.relatedIndustries.map((ri) => (
              ...
              >
                <img src={ri.image} alt={ri.name} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  <span className="text-sm font-semibold text-white">{ri.name}</span>
                </div>
              </Link>
            ))}
          </MobileRail>
